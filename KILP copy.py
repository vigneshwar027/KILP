...
#     if date in ["0100-01-01","0100/01/01"]: 
#         date = ''
...
def secure_tables():
    ...
    # for  table in tables:
    #     table_columns = cursor.execute(''' set quoted_identifier off;
    ...
    #             print('hi')
    #             cursor.execute(''' set quoted_identifier off; UPDATE {}
        ...
        # if True:
            ...
            if petitioner_xref and petitioner_name:
                ...
                if len(petitioners) <= 0 :
                    cursor.execute( 
                            ''' set quoted_identifier off; INSERT INTO dbo.Petitioner(PetitionerXref, PetitionerName) VALUES ("{}", "{}") '''.format(
            ...
            if True: # if petitioner id condition is replaced as if true
                ...
                if "beneficiary_record_opened_date" in list_h and str(
                        ...
                        row["beneficiary_record_opened_date"]) and not '0100-01-01' and not '1900-01-01 00:00:00.000':
                ...
                if "i94_valid_from" in list_h and str(row["i94_valid_from"]).strip() and not pd.isna(row["i94_valid_from"]):
                    ...
                    immigration_status_valid_from = change_format(["i94_valid_from"])
...
def process_case_file(file_path, from_name):
    ...
    for index, row in df.iterrows():
        ...
        if True:
            ...
            if organization_xref and organization_name:
                results = cursor.execute(
                    ''' set quoted_identifier off; SELECT * FROM dbo.Organization where OrganizationXref="{}" '''.format(organization_xref)).fetchall()
                length = len(results)
                if length <= 0:
                    cursor.execute(
                        ''' set quoted_identifier off; INSERT INTO dbo.Organization(OrganizationXref, OrganizationName) VALUES ("{}", "{}") '''.format(
                            organization_xref, organization_name))
                    cursor.commit()
                    ##print('inserted')
            ...
            is_primary_beneficiary = 1
            if petitioner_xref and petitioner_name:
                petitioners = cursor.execute(''' set quoted_identifier off; select * from dbo.Petitioner where PetitionerXref="{}" '''.format(petitioner_xref)).fetchall()

                if len(petitioners) <= 0 :
                    cursor.execute(''' set quoted_identifier off; INSERT INTO dbo.Petitioner(PetitionerXref, PetitionerName, OrganizationXref) VALUES ("{}", "{}", "{}") '''.format(petitioner_xref, petitioner_name,organization_xref))
                    cursor.execute("SELECT @@IDENTITY AS ID;")
                    petitioner_id = cursor.fetchone()[0]
                    cursor.commit()
...
def start():
    ...
    result_filepath6 = 'Processed Reports/Comcast Dashboard Report_'+str(todate)+'.xlsx'
    ComcastDashBoardReport(result_filepath6)
...
def ComcastReport(result_filepath):
    ...
    where b.IsActive=1 and lower(c.IsCurrentProcess) = 'true' and p2.PetitionerXref = '625365045' order by b.PrimaryBeneficiaryFullName asc ,  b.BeneficiaryType desc '''
